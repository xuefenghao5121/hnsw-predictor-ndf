...
<!-- ════════════════════════════════════════════════════════════════ -->
<!-- deduced 条款:基于 DDD/六边形架构的目标模块重设计             -->
...
## `models/` 与 `poc/` 边界（adopted）

> **非 SoT 正文** — adopted 指针。Canonical:
> [`../meta/architecture.md#ARCH-008`](../meta/architecture.md#ARCH-008)（ARCH-008）。  
> 见 [`../meta/decisions/adr-meta-layer-split.md`](../meta/decisions/adr-meta-layer-split.md)。
> 摘要：`models/` = L3 金标；探索进 `poc/<topic>/` + 装订器 `ndf/`；`poc.sot: false`。
