...
def compile_plan(
    *,
    ...
    if task in {
        ...
        "poc_implementation",
        "poc_prepare_baseline",
    } and "implementation_approval" not in approved and "bundle_dispatch" not in approved:
        plan["human_phrase"] = "派发"
...
def verify_plan(
    ...
    *,
    ...
    if required_gate:
        # Text-first 「派发」 (bundle_dispatch) is the ADR-META-003 substitute for
        # classic implementation_approval; same review-slice SHA contract.
        implementation = [
            item
            for item in plan.get("gates", {}).get("receipts", [])
            if item.get("gate") in {"implementation_approval", "bundle_dispatch"}
