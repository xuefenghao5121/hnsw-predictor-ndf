...
def poc_dispatch(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "task": task,
        "intent": intent,
        "provider": None,  # filled from ndf.workflow.yaml role binding below
    ...
    try:
        ...
        resolved = role_binding.resolve_role(wf.ROOT, "implementation")
        payload["provider"] = str(
            resolved.get("provider") or "openclaw"
        )
        payload["role_resolution"] = resolved
    except Exception:
        payload["provider"] = "openclaw"
    # OpenClaw dispatch requires session_key (same Feishu direct as Control).
    payload["session_key"] = wf.openclaw_session_key() or payload.get("session_key")
