...
| 开发 track | harness 用法 |
|------------|----------------|
| **bootstrap** | `ndf_workflow_status genesis-status/genesis-pack`；Foundation gate 后 Claude Code Trunk candidate；不造性能金标 |
...
| 已落地 | 明确不做 / 后续 |
|--------|-----------------|
| graph + bind 双 Linter | Linter 自动修图 |
| Workflow snapshot + Canvas Skill（[[META-009]]…[[META-011]]） | Canvas 充当第五 SoT |
