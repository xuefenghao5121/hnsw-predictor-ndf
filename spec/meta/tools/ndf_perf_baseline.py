...
def inspect_topic(topic: str, require_card: bool) -> CardView:
    ...
    # Empty / missing status fields are pending — not "unverified". Never IndexError.
    if _first_token(view.status) == "unverified" or _first_token(evidence_status) == "unverified":
        view.findings.append(
            Finding(
                "error",
