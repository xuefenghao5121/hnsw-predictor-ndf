...
def _pack_worker_contract_errors(pack: Mapping[str, Any], message: str) -> list[str]:
    """Fail closed if the worker prompt would drop Genesis intent (pageANN incident)."""
    errors: list[str] = []
    request = pack.get("request") if isinstance(pack.get("request"), Mapping) else {}
    intent = str(request.get("intent") or "").strip()
    if not intent:
...
def extract_dispatch_notify(text: str | None) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    for field in ("result", "receipt_path", "topic", "task", "episode_id", "attempt_id"):
        ...
        elif field in {"episode_id", "attempt_id"}:
            # May be empty for binder/control hops without Episode binding (META-011).
            continue
...
def _all_write_roots(pack: Mapping[str, Any]) -> list[str]:
    raw = pack.get("allowed_write_roots") or pack.get("allowed_write_paths")
    if raw is None and pack.get("allowed_write_root"):
        raw = [pack.get("allowed_write_root")]
    if isinstance(raw, str):
        raw = [raw]
    if not isinstance(raw, list):
        return []
    return [_normalize_relpath(str(item)) for item in raw if str(item).strip()]


def _completion_anchor(pack: Mapping[str, Any]) -> str:
    """Directory where completion receipts may land (never a binder .md file path)."""
    topic = str(pack.get("topic") or "").strip()
    topic_ndf = ""
    workspace = pack.get("workspace") if isinstance(pack.get("workspace"), Mapping) else {}
    ...
    roots = _all_write_roots(pack)
    if topic_ndf:
        ndf = topic_ndf.rstrip("/")
        for root in roots:
            r = root.rstrip("/")
            if r == ndf or r.startswith(ndf + "/") or ndf.startswith(r + "/"):
                return ndf
    first = _first_write_root(pack)
    if first.lower().endswith((".md", ".json", ".yaml", ".yml", ".txt")):
        parent = str(Path(first).parent).replace("\\", "/")
        return _normalize_relpath(parent)
    return first
...
def completion_receipt_path_for_pack(pack: Mapping[str, Any]) -> str:
    ...
    if declared:
        # Repair legacy packs that anchored under a binder .md file path.
    ...
    if not topic_ndf and topic and str(pack.get("track") or "") != "bootstrap":
        topic_ndf = f"poc/{topic}/ndf"
    anchor = _completion_anchor(pack)
    if topic_ndf:
        candidate = _normalize_relpath(
            f"{topic_ndf.rstrip('/')}/evidence/{task}{hop_part}-completion.json"
        )
        if not anchor or _under_write_root(candidate, anchor) or _under_write_root(
            candidate, topic_ndf
        ):
            return candidate
    tmp_candidate = _normalize_relpath(f"tmp/ndf-completion/{attempt}{hop_part}.json")
    if anchor and _under_write_root(tmp_candidate, anchor):
        return tmp_candidate
    if anchor:
        return _normalize_relpath(
            f"{anchor.rstrip('/')}/.ndf-completion/{task}{hop_part}-{attempt}.json"
        )
    return tmp_candidate
...
def _resolve_disk_receipt_path(
    ...
    write_root = _completion_anchor(pack) or _first_write_root(pack)
...
def _effective_dispatch_provider(pack: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    # Pack may already select fallback (e.g. gateway down → in-host). Honor it.
    if declared in {"in-host", "dual-session", "custom"}:
        task = str(pack.get("task") or "")
        track = str(pack.get("track") or "")
        if (
            task.startswith("poc_")
            or task in {"implement", "poc_measurement", "prepare_acp_lease"}
            or (track == "poc" and "binder" not in task and "gate" not in task and "proposal" not in task)
        ):
            role = "implementation"
        else:
            role = "control"
        resolved = role_binding.resolve_role(ROOT, role)
        return declared, {**resolved, "provider": declared, "mapped_role": role}
...
def _prepare_isolated_lease(
    ...
    *,
    ...
    # Text-first / ADR-META-004: Episode is optional. Mint a local lease id when
    # absent so isolated worktree prep does not reintroduce Replay coupling.
    stamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime())
    if not episode_id and topic:
        episode_id = f"ep-inline-{topic}-{stamp}"
    missing = [
        name
        for name, value in (
            ("topic", topic),
    ...
    try:
        ...
        # Text-first / ADR-META-004: record lease on disk only. Do not call
        # record_runtime_lease (ReplayStore retired; Episode optional).
        ...
        pack_sha = str(pack.get("pack_sha") or "").strip()
        if not pack_sha:
            pack_sha = workflow.canonical_json_sha(
                {
                    "topic": topic,
        ...
        lease = {
            ...
            "step": "start",
            "repo_head": base_sha,
            "source_generation_sha": workflow.source_generation_sha(),
            "manifest_sha": pack.get("manifest_sha"),
            "context_plan_sha": pack.get("plan_sha"),
            "command": "runtime lease (text-first inline)",
            "input_sha": workflow.canonical_json_sha(seed),
        ...
        workflow.append_lease(workflow.LEASE_LOG, lease, root=workflow.ROOT)
        return {
            "ok": True,
            "transport_ok": True,
            "state": "succeeded",
    ...
    *,
    ...
    if provider == "openclaw":
        ...
        if (
            ...
            # Prefer yaml fallback when primary OpenClaw transport fails (gateway down etc.).
            # Do not silently demote Implementation POC to in-host when pack asked openclaw —
            # that collapses authorship (human rejected in-host for this project path).
            import ndf_role_binding as role_binding

            mapped = str(role_resolution.get("mapped_role") or role_resolution.get("role") or "control")
