#!/usr/bin/env python3
"""Content-addressed Agent Episode recording (library only).

CLI is deprecated (ADR-META-004): ``python3 ndf_replay.py …`` exits 2.
Historical store ``<repo>/.ndf/replay`` is archaeology, not NDF SoT.
Daily hot path MUST NOT write new episodes or button-actions.
"""
...
def main(argv: list[str] | None = None) -> int:
    """CLI retired (ADR-META-004). Library imports remain for archaeology/tests."""
    del argv
    print(
        "ndf_replay.py CLI is deprecated (ADR-META-004). "
        "Episode/Replay/Guest are retired from the NDF hot path. "
        "Historical data under .ndf/replay/ is read-only archaeology.",
        file=sys.stderr,
    )
    return 2
