# Decisions — P2 (DEC-034…039)

> 条款索引: `DEC-034`, `DEC-035`, `DEC-036`, `DEC-037`, `DEC-038`, `DEC-039`

## D-034: VisitedList uint32 -> uint8 内存优化 {#DEC-034}
<!-- ndf: kind=decision date=2026-07-30 status=stable affects=BEH-002,CON-002 source=observed -->
...
<!-- ndf: kind=decision date=2026-07-30 status=stable affects=DEC-003,DEC-009,BEH-007 source=observed -->
...
<!-- ndf: kind=decision date=2026-07-30 status=stable affects=CHR-006,CHR-004,CON-007 source=observed -->
...
<!-- ndf: kind=decision date=2026-07-30 status=stable since=0.4 affects=DEC-021,DEC-059,CON-HONEST-002 source=deduced -->
...
2. 性能报告 MUST 至少给出 Buffered 与 Direct 两组数字，或明确标注单模式及其局限
   （契约化见 [[CON-HONEST-002]]）。
3. 本协议的执行证据与加速比量化见 [[DEC-057]]；O_DIRECT 作为**性能地板 / 优化基座**
   的战略定位见 [[DEC-059]]（[[DEC-030]] 已 superseded）。

**Alternatives rejected.**
- 只报 Buffered：违反诚实协议，掩盖 page cache 加速比
- 废弃 Buffered：page cache 在 cgroup 预算内合法，仍为生产默认路径

> rationale: 诚实协议先于 O_DIRECT 加速比量化落盘；`DEC-039` 填补
