...
#   <前缀>_blocks_${BS_NAME}k.bin  <前缀>_route_${BS_NAME}k.bin
#   <前缀>_vecblocks_${BS_NAME}k.bin  <前缀>_vecblocks_${BS_NAME}k_route.bin
#   pqco_<前缀>_M<M>.bin
...
BS=${BS:-65536}  # 64KB default, override with env
BS_NAME=$((BS / 1024))
O=output
mkdir -p "$O" data
...
./build/write_blocks_veconly "$O/${PREFIX}_graph.bin" "$O/${PREFIX}_bfs.bin" \
    "$O/${PREFIX}_vecblocks_${BS_NAME}k.bin" $BS

echo "[5/7] write_blocks + gen_route ..."
./build/write_blocks "$O/${PREFIX}_graph.bin" "$O/${PREFIX}_bfs.bin" \
    "$O/${PREFIX}_blocks_${BS_NAME}k.bin" $BS
./build/gen_route "$O/${PREFIX}_blocks_${BS_NAME}k.bin" "$O/${PREFIX}_route_${BS_NAME}k.bin"
