...
// ============================================================
struct VisitedList {
    std::vector<uint8_t> mass;   // uint8: 10M 节点省 30MB vs uint32 (DEC-064)
    uint8_t curV;                // 当前标记值
...
private:
    ...
    // mmap-budget-shift R0: mmap'd PQ codes (file-backed, cgroup file budget)
    uint8_t* pq_mmap_data_ = nullptr;  // points to codes (after header)
    size_t pq_mmap_size_ = 0;
    int pq_mmap_fd_ = -1;
