...
上游参考（**非 SoT**）：[normative_language](https://github.com/hengliao1972/normative_language)。
本仓 NDF 条款语言 SoT：[`meta/language.md`](meta/language.md)（[[META-001]]…[[META-003]]）。
树按**子系统归属**，语义靠条款 ID 图，历史靠 git。

## 读序（按角色）

| 角色 | 先读 |
|------|------|
| 指挥 / 流程 | [`../AGENTS.md`](../AGENTS.md) → [`meta/README.md`](meta/README.md) → [`meta/language.md`](meta/language.md) → [`meta/process.md`](meta/process.md) |
...
| 目录 | 角色 | 本仓库文件 |
|------|------|------------|
| **`meta/`** | **NDF process profile**（语言 + 流程 SoT；非产品行为） | `language`, `process`, `architecture`, `constraints`, `glossary`, `decisions/`, `open/`, **`tools/`** |
