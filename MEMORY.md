...
| 最近工作 | meta 分层 `6809f2a`；Trunk `open/` 卫生收口 r2 |
...
| SLA / 约束 | `spec/40-constraints/sla.md`（[[CON-SLA-014]]；[[CON-POC-001]] 在 meta） |
...
| [[DEC-066]] | 废止白嫖 QPS；must vs 观测基线（假基线已修正） | Accepted |
...
| 任务 | 状态 | 关联 | 备注 |
| :--- | :--- | :--- | :--- |
| L4 page cache | exploring | `poc/l4-cache-mgmt/`、draft [[BEH-024]] | EvictMeta +3%；FADVISE 有害；待关闭决策 |
| I/O pipelining | exploring | `poc/io-pipelining/`、BEH-021…023 / API-010 / CON-SLA-013 | 等 correction 重测；依赖 L4 |

### 3.3 待处理事项

- [ ] **L4 关闭决策**：promote EvictMeta 或 FADVISE 负结果（[[BEH-020]]）— 高
- [ ] **io-behavior-correction**：确认 Pending 后严格隔离重测 — 高
- [ ] **4T / DEEP10M** 严格基线补全 — 中
- [ ] **推远程** — 低
...
| 阻塞项 | 原因 | 依赖 |
| :--- | :--- | :--- |
| pipe promote | 旧口径作废 | L4 或 C 组 + behavior 重测 |
...
| L4 R0–R3 v2 | `poc/l4-cache-mgmt/ndf/` |
...
- **2026-08-03**: BEH-025 装订器；DEC-065/066；DEC-066 假基线修正  
- **更早**: DEC-061 RC 负结果  
...
| topic | draft 条款 | 下一闸门 |
| :--- | :--- | :--- |
| `l4-cache-mgmt` | BEH-024 | promote / reject |
| `io-pipelining` | BEH-021…023, API-010, CON-SLA-013 | correction 重测 |


## 6. 下次会话启动引导

- **节奏**: 先确认 open 够薄 → 只动 POC → 关闭才回合 Trunk。  
- **L4**: `poc/l4-cache-mgmt/ndf/TOPIC.md`  
- **pipe**: `proposal-io-behavior-correction.md` + io TOPIC  
- **process**: `spec/meta/open/proposal-meta-*.md`  
- **审核**: `python3 spec/meta/tools/ndf_index.py …`
