...
void DiskHNSW::loadPQCodes(const std::string& pq_path) {
    // === mmap-budget-shift R0: mmap BFS-order PQ codes ===
    const char* mmap_path = std::getenv("PQ_MMAP_PATH");
    if (mmap_path) {
        std::cout << "[DiskHNSW] Loading PQ codes via mmap from " << mmap_path << "..." << std::endl;

        int fd = open(mmap_path, O_RDONLY);
        if (fd < 0) {
            throw std::runtime_error("Cannot open mmap PQ file: " + std::string(mmap_path));
        }

        // Read header: BPQC + n + M
        char magic[4];
        read(fd, magic, 4);
        if (std::memcmp(magic, "BPQC", 4) != 0) {
            throw std::runtime_error("Invalid mmap PQ magic (expected BPQC)");
        }
        uint64_t n;
        uint32_t M;
        read(fd, reinterpret_cast<char*>(&n), sizeof(uint64_t));
        read(fd, reinterpret_cast<char*>(&M), sizeof(uint32_t));

        // mmap the entire file (offset must be page-aligned)
        // Header is 16B, but we mmap from offset 0 and skip header in pointer
        size_t codes_size = n * M;
        size_t total_size = 16 + codes_size;  // header + codes
        void* mapped = mmap(nullptr, total_size, PROT_READ, MAP_PRIVATE, fd, 0);
        if (mapped == MAP_FAILED) {
            close(fd);
            throw std::runtime_error("mmap failed for PQ codes: " + std::string(strerror(errno)));
        }
        madvise(mapped, total_size, MADV_RANDOM);

        // Skip 16B header to get to codes region
        pq_mmap_data_ = static_cast<uint8_t*>(mapped) + 16;
        pq_mmap_size_ = codes_size;
        pq_mmap_fd_ = fd;
        pq_mmap_base_ = mapped;  // keep base for munmap
        pq_mmap_total_ = total_size;

        // Set pq_params from original PQ file header (need codebook)
        // Read codebook from original file
        std::ifstream cb_in(pq_path, std::ios::binary);
        if (!cb_in.is_open()) {
            throw std::runtime_error("Cannot open original PQ file for codebook: " + pq_path);
        }
        char cb_magic[4];
        cb_in.read(cb_magic, 4);
        uint64_t cb_n;
        uint32_t cb_M, cb_nbits, cb_dim;
        cb_in.read(reinterpret_cast<char*>(&cb_n), sizeof(uint64_t));
        cb_in.read(reinterpret_cast<char*>(&cb_M), sizeof(uint32_t));
        cb_in.read(reinterpret_cast<char*>(&cb_nbits), sizeof(uint32_t));
        cb_in.read(reinterpret_cast<char*>(&cb_dim), sizeof(uint32_t));
        uint32_t codebook_M, codebook_K, codebook_dsub;
        cb_in.read(reinterpret_cast<char*>(&codebook_M), sizeof(uint32_t));
        cb_in.read(reinterpret_cast<char*>(&codebook_K), sizeof(uint32_t));
        cb_in.read(reinterpret_cast<char*>(&codebook_dsub), sizeof(uint32_t));

        pq_params_.M = cb_M;
        pq_params_.nbits = cb_nbits;
        pq_params_.dim = cb_dim;
        pq_params_.dsub = codebook_dsub;
        pq_params_.ksub = codebook_K;

        // Read codebook
        size_t codebook_size = (size_t)codebook_M * codebook_K * codebook_dsub;
        pq_codebook_.resize(codebook_size);
        cb_in.read(reinterpret_cast<char*>(pq_codebook_.data()), codebook_size * sizeof(float));
        cb_in.close();

        // Override pq_codes_ to use mmap pointer via placement
        // We need pq_codes_ to reference mmap'd memory. Since pq_codes_ is std::vector<uint8_t>,
        // we can't easily make it point to external memory. Instead, we'll use a wrapper.
        // For R0 simplicity: set a flag and use the mmap pointer in pqDistance.
        pq_codes_mmap_ = true;

        std::cout << "  PQ params: N=" << n << ", M=" << cb_M << ", nbits=" << cb_nbits
                  << ", dim=" << cb_dim << ", dsub=" << codebook_dsub
                  << ", ksub=" << codebook_K << std::endl;
        std::cout << "  [mmap] PQ codes mapped: " << (codes_size / 1024 / 1024) << "MB (file-backed, cgroup file budget)" << std::endl;

        pq_enabled_ = true;

        // DEC-034: free upper vectors
        static const bool kUpperPQ = !std::getenv("DISABLE_UPPER_PQ");
        if (kUpperPQ && !graph_.upper_vectors.empty()) {
            ...
            malloc_trim(0);
            std::cout << "  [UpperPQ] Freed " << upper_count << " exact vectors (" << upper_mb << "MB)" << std::endl;
        }

        size_t codebook_mb = codebook_size * sizeof(float) / (1024 * 1024);
        std::cout << "  PQ codebook: " << codebook_mb << " MB (anon)" << std::endl;
        std::cout << "  PQ codes: " << (codes_size / 1024 / 1024) << " MB (mmap/file-backed)" << std::endl;
        std::cout << "  PQ enabled: ADC distance will be used for Layer 0" << std::endl;
        return;
    }
    // === end mmap-budget-shift R0 ===
...
float DiskHNSW::pqDistance(const float* query, uint32_t node_id_new) const {
    ...
    const uint8_t* code;
    if (pq_codes_mmap_) {
        code = &pq_mmap_data_[(size_t)node_id_new * pq_params_.M];
    } else {
        code = &pq_codes_[(size_t)node_id_new * pq_params_.M];
    }
            ...
            for (size_t j = 0; j < local_neighbors.size(); j++) {
                ...
                if (kPrefetchSW2 && j + kPfDist2 < local_neighbors.size()) {
                    ...
                    if (pfn < graph_.num_nodes) {
                        ...
                        if (pq_enabled_) {
                            SIMD_PREFETCH(pq_codes_mmap_ ? &pq_mmap_data_[(size_t)pfn * pq_params_.M] : &pq_codes_[(size_t)pfn * pq_params_.M]);
