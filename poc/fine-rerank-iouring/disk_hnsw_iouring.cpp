...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            // POC fine-rerank-iouring: per-thread io_uring (buffered) 替代 WILLNEED_BG + pread
            // R1: 纯 io_uring (FINE_IOURING=1)
            // R2: 串行混合 fadvise+io_uring (FINE_IOURING=1 IOURING_HYBRID=1)
            // R3: 异步并行 BG fadvise + io_uring (FINE_IOURING=1 IOURING_ASYNC=1)
            static const bool kFineIouring = std::getenv("FINE_IOURING") && std::atoi(std::getenv("FINE_IOURING")) != 0;
            static const bool kIouringHybrid = std::getenv("IOURING_HYBRID") && std::atoi(std::getenv("IOURING_HYBRID")) != 0;
            static const bool kIouringAsync = std::getenv("IOURING_ASYNC") && std::atoi(std::getenv("IOURING_ASYNC")) != 0;
            if (kFineIouring && !pages_needed.empty() && vec_blocks_fd_ >= 0) {
                // ---- per-thread io_uring buffered 路径 ----
                // R3: BG 线程 fadvise + 搜索线程 io_uring 并行
                // buffered 模式: 页进 page cache, 跨 query 复用不变
                auto tp0 = std::chrono::high_resolution_clock::now();

                // R3: 异步 BG fadvise - 复用 WILLNEED_BG SPSC slot 机制
                // BG 线程并行提交 fadvise, 搜索线程立即开始 io_uring 读取
                if (kIouringAsync) {
                    // 复用现有 WILLNEED_BG 的 SPSC slot 架构
                    static constexpr int MAX_THREADS = 128;
                    struct BgSlot {
                        std::atomic<bool> ready{false};
                        std::vector<uint32_t> pages;
                    ...
                    for (uint32_t pg : pages_needed) slot.pages.push_back(pg);
                    slot.ready.store(true, std::memory_order_release);
                    // BG 线程现在并行做 fadvise, 搜索线程继续做 io_uring
                }

                // R2: 串行 fadvise (仅 hybrid 模式)
                if (kIouringHybrid && !kIouringAsync) {
                    std::vector<uint32_t> sorted_pages(pages_needed.begin(), pages_needed.end());
                    std::sort(sorted_pages.begin(), sorted_pages.end());
                    size_t j = 0;
                    while (j < sorted_pages.size()) {
                        uint32_t start = sorted_pages[j];
                        size_t end = j + 1;
                        while (end < sorted_pages.size() && sorted_pages[end] == sorted_pages[end-1] + 1) end++;
                        posix_fadvise(vec_blocks_fd_, (off_t)start << 12, (end - j) << 12, POSIX_FADV_WILLNEED);
                        j = end;
                    }
                }

                // per-thread io_uring 实例 (thread_local, 无共享状态)
                static thread_local std::unique_ptr<IoUring> tls_ring;
                if (!tls_ring) {
                    tls_ring = std::make_unique<IoUring>(256, 0);
                    tls_ring->setBufferSize(4096);
                }
                auto& ring = *tls_ring;

                // 分配 buffer 并提交所有页
                std::unordered_map<uint32_t, int> page_to_buf;
                page_to_buf.reserve(pages_needed.size());
                int submitted = 0;
                for (uint32_t pg : pages_needed) {
                    int buf = ring.allocBuffer();
                    if (buf < 0) {
                        ring.flushSqe();
                        ring.submit();
                        std::vector<IoUring::CqeResult> results;
                        ring.reapCompletions(results);
                        for (auto& r : results) ring.freeBuffer(r.user_data >> 1);
                        buf = ring.allocBuffer();
                        if (buf < 0) break;
                    }
                    ring.submitReadNF(vec_blocks_fd_, (off_t)pg << 12, 4096, buf, (uint64_t)pg);
                    page_to_buf[pg] = buf;
                    submitted++;
                }
                ring.flushSqe();
                ...
                // 按完成顺序处理: reap CQE, 对每个就绪页处理其上的候选
                std::unordered_set<uint32_t> processed_pages;
                std::unordered_set<uint32_t> processed_cands;  // 防止重复处理
                int remaining = submitted;
                char tmp_vec_iouring[512];

                // 预建 page -> candidates 映射, 避免每次全量扫描 io_cands
                ...
                while (remaining > 0) {
                    ring.waitCompletion();
                    std::vector<IoUring::CqeResult> results;
                    int n = ring.reapCompletions(results);
                    for (auto& res : results) {
                        uint32_t pg = (uint32_t)res.user_data;
                        processed_pages.insert(pg);
                        remaining--;

                        // 处理此页上的候选
                        auto it = page_to_cands.find(pg);
                        if (it != page_to_cands.end()) {
                            const char* p0 = (const char*)ring.getBuffer(page_to_buf[pg]);
                            for (int ci : it->second) {
                                if (processed_cands.count(ci)) continue;
                                const auto& c = io_cands[ci];
                                const float* vec;
                                if (!c.cross) {
                                    vec = reinterpret_cast<const float*>(p0 + c.oip);
                                } else if (processed_pages.count(c.page0 + 1)) {
                                    const char* p1 = (const char*)ring.getBuffer(page_to_buf[c.page0 + 1]);
                                    size_t first = 4096 - c.oip;
                                    std::memcpy(tmp_vec_iouring, p0 + c.oip, first);
                                    std::memcpy(tmp_vec_iouring + first, p1, dim_ * sizeof(float) - first);
                                    vec = reinterpret_cast<const float*>(tmp_vec_iouring);
                                } else {
                                    continue;  // 跨页第二页未就绪
                                }
                                consider(c.nid, vec);
                                cache_->putFlatVector(c.nid, vec);
                                processed_cands.insert(ci);
                            }
                        }
                    }
                    // 检查是否有跨页候选的第二页已就绪
                    for (int ci = 0; ci < (int)io_cands.size(); ci++) {
                        if (processed_cands.count(ci)) continue;
                        const auto& c = io_cands[ci];
                        if (c.cross && processed_pages.count(c.page0) && processed_pages.count(c.page0 + 1)) {
                            const char* p0 = (const char*)ring.getBuffer(page_to_buf[c.page0]);
                            const char* p1 = (const char*)ring.getBuffer(page_to_buf[c.page0 + 1]);
                            size_t first = 4096 - c.oip;
                            std::memcpy(tmp_vec_iouring, p0 + c.oip, first);
                            std::memcpy(tmp_vec_iouring + first, p1, dim_ * sizeof(float) - first);
                            consider(c.nid, reinterpret_cast<const float*>(tmp_vec_iouring));
                            cache_->putFlatVector(c.nid, reinterpret_cast<const float*>(tmp_vec_iouring));
                            processed_cands.insert(ci);
                        }
                    }
                }
                // 释放所有 buffer
                for (auto& [pg, buf] : page_to_buf) {
                    ring.freeBuffer(buf);
                }
                ...
                if (kProfile) {
                    double a = std::chrono::duration<double, std::micro>(tp1 - tp0).count();
                    double w = std::chrono::duration<double, std::micro>(tp2 - tp1).count();
                    acc_a += a; acc_wait += w; acc_rerank += 0; acc_n++;
                    if (acc_n % 200 == 0) {
                        fprintf(stderr, "[PROFILE_TS] n=%ld iouring_submit=%.0fus iouring_wait=%.0fus\n",
