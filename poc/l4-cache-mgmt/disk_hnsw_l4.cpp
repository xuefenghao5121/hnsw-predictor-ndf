...
// D1: flat_vec_cache hit/miss diagnosis
std::atomic<uint64_t> g_fv_hits{0};
std::atomic<uint64_t> g_fv_misses{0};
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            for (uint32_t nid : cand_ids) {
                ...
                if (const float* fv = cache_->getFlatVector(nid)) { consider(nid, fv); g_fv_hits++; continue; }
                g_fv_misses++;
            ...
            if (kL4Willneed && !pages_needed.empty() && vec_blocks_fd_ >= 0) {
                if (kWillneedBg) {
                    ...
                    static std::thread bg_thread([&]() {
                        ...
                        static const bool kPageMerge = std::getenv("PAGE_MERGE_BG") && std::atoi(std::getenv("PAGE_MERGE_BG")) != 0;
                        while (!bg_stop.load(std::memory_order_relaxed)) {
                            for (int i = 0; i < MAX_THREADS; i++) {
                                if (bg_slots[i].ready.load(std::memory_order_acquire)) {
                                    if (kPageMerge && bg_slots[i].pages.size() > 1) {
