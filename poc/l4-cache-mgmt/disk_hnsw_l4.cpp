...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            // L4 POC R5a: WILLNEED - hint kernel to prefetch pages before reading
            ...
            //
            // L4 POC R5b: L4_SELECTIVE_DONTNEED=1 时只驱逐不含结果向量的冷页
            static const bool kFineFadvise = std::getenv("FINE_FADVISE") && std::atoi(std::getenv("FINE_FADVISE")) != 0;
            static const bool kL4SelectiveDontneed = std::getenv("L4_SELECTIVE_DONTNEED") && std::atoi(std::getenv("L4_SELECTIVE_DONTNEED")) != 0;
            static const bool kFineBufferedInner = std::getenv("FINE_BUFFERED") && std::atoi(std::getenv("FINE_BUFFERED")) != 0;
            if (kFineBufferedInner && !pages_needed.empty() && vec_blocks_fd_ >= 0) {
                if (kL4SelectiveDontneed) {
                    // R5b: 收集 top_candidates 中的 result nids
                    std::set<uint32_t> top_nids;
                    { auto tc_copy = top_candidates; while (!tc_copy.empty()) { top_nids.insert(tc_copy.top().second); tc_copy.pop(); } }
                    // 构建 page -> nids 反向映射
                    std::unordered_map<uint32_t, std::set<uint32_t>> page_to_nids;
                    for (const auto& c : io_cands) {
                        page_to_nids[c.page0].insert(c.nid);
                        if (c.cross) page_to_nids[c.page0 + 1].insert(c.nid);
                    }
                    // 选择性驱逐: 不含任何 result nid 的页才 DONTNEED
                    size_t evicted = 0, kept = 0;
                    for (uint32_t pg : pages_needed) {
                        bool has_result = false;
                        auto it = page_to_nids.find(pg);
                        if (it != page_to_nids.end()) {
                            for (uint32_t nid : it->second) {
                                if (top_nids.count(nid)) { has_result = true; break; }
                            }
                        }
                        if (!has_result) {
                            posix_fadvise(vec_blocks_fd_, (off_t)pg << 12, 4096, POSIX_FADV_DONTNEED);
                            evicted++;
                        } else { kept++; }
                    }
                    static long sd_total = 0, sd_evicted = 0, sd_kept = 0, sd_n = 0;
                    sd_total += pages_needed.size(); sd_evicted += evicted; sd_kept += kept; sd_n++;
                    if (sd_n % 200 == 0) {
                        fprintf(stderr, "[SELECTIVE_DONTNEED] n=%ld total=%.1f evicted=%.1f kept=%.1f\n",
                                sd_n, (double)sd_total/sd_n, (double)sd_evicted/sd_n, (double)sd_kept/sd_n);
                    }
                } else if (kFineFadvise) {
                    // R1 原始 blanket 驱逐: 驱逐所有 fine rerank 页
