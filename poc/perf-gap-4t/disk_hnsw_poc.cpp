...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            if (kFinePread) {
                // ---- D3: batch pread via readv (reduce syscall count) ----
                // ---- pread 路径 (线程安全, 多线程并发用) ----
                auto tp0 = std::chrono::high_resolution_clock::now();
                std::unordered_map<uint32_t, std::unique_ptr<char[]>> page_cache;
                page_cache.reserve(pages_needed.size());

                // D3: sort pages and use readv for batch submission
                static const bool kBatchPread = std::getenv("BATCH_PREAD") && std::atoi(std::getenv("BATCH_PREAD")) != 0;
                if (kBatchPread && pages_needed.size() > 1) {
                    // Sort pages for readv
                    ...
                    iovs.reserve(sorted_pages.size());
                    bufs.reserve(sorted_pages.size());
                    ...
                    for (uint32_t pg : sorted_pages) {
                        ...
                        iovs.push_back({raw_ptr, 4096});
                        bufs.push_back(std::move(buf));
                    }

                    // Use preadv2 to submit all reads at once (RWF_NOWAIT optional)
                    // Fall back to sequential pread if preadv fails
                    ssize_t total_read = preadv(vec_blocks_fd_, iovs.data(), iovs.size(),
                                               (off_t)sorted_pages[0] << 12);
                    // Note: preadv requires contiguous offsets, so for non-contiguous pages
                    // we use individual pread but sorted (benefits elevator scheduler)
                    if (total_read < 0) {
                        // preadv failed (non-contiguous), fall back to sorted individual pread
                        for (size_t i = 0; i < sorted_pages.size(); i++) {
                            ssize_t r = pread(vec_blocks_fd_, bufs[i].get(), 4096,
                                            (off_t)sorted_pages[i] << 12);
                            if (r == 4096) page_cache[sorted_pages[i]] = std::move(bufs[i]);
                        }
                    } else {
                        // preadv succeeded - but we need to verify each page was read
                        // For non-contiguous, preadv reads from the SAME starting offset sequentially
                        // This won't work for scattered pages. Use sorted individual pread instead.
                        for (size_t i = 0; i < sorted_pages.size(); i++) {
                            ssize_t r = pread(vec_blocks_fd_, bufs[i].get(), 4096,
                                            (off_t)sorted_pages[i] << 12);
                            if (r == 4096) page_cache[sorted_pages[i]] = std::move(bufs[i]);
                ...
                // Original path: individual pread per page
                for (uint32_t pg : pages_needed) {
