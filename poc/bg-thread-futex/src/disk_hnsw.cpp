...
// futex constants (if not available via headers)
...
// futex helpers for bg_thread optimization (POC: bg-thread-futex)
static inline int futex_wait_priv(int* uaddr, int val) {
    return syscall(SYS_futex, uaddr, FUTEX_WAIT_PRIVATE, val, nullptr, nullptr, 0);
}
static inline int futex_wake_priv(int* uaddr, int val) {
    return syscall(SYS_futex, uaddr, FUTEX_WAKE_PRIVATE, val, nullptr, nullptr, 0);
}
#include <mutex>
#include <array>
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            if (kL4Willneed && !pages_needed.empty() && vec_blocks_fd_ >= 0) {
                if (kWillneedBg) {
                    // D-futex: SPSC per-thread slot + atomic flag + futex wake
                    // POC: bg-thread-futex (replaces sched_yield with futex block/wake)
                    ...
                    // Futex word: searchKnn increments to signal new work
                    static std::atomic<int> bg_wake_seq{0};
                    // Dynamic scan limit: tracks max active slot index
                    static std::atomic<int> bg_active_slots{0};
                    ...
                    static std::thread bg_thread([&]() {
                        ...
                        // Lambda: process ready slots, return true if any work done
                        auto process_slots = [&](int scan_limit) -> bool {
                            bool found = false;
                            for (int i = 0; i < scan_limit; i++) {
                                if (!bg_slots[i].ready.load(std::memory_order_acquire)) continue;
                        ...
                        while (!bg_stop.load(std::memory_order_relaxed)) {
                            int scan_limit = std::max(1, bg_active_slots.load(std::memory_order_relaxed));
                            // First scan
                            if (process_slots(scan_limit)) continue;
                            // No work — double-check with futex race protection
                            int saved_seq = bg_wake_seq.load(std::memory_order_acquire);
                            if (process_slots(scan_limit)) continue;
                            // Confirmed no work — block on futex
                            futex_wait_priv(reinterpret_cast<int*>(&bg_wake_seq), saved_seq);
                        }
                    });
                    ...
                    // Update active slot count for dynamic scan
                    {
                        int prev = bg_active_slots.load(std::memory_order_relaxed);
                        while (my_slot + 1 > prev &&
                               !bg_active_slots.compare_exchange_weak(prev, my_slot + 1,
                    ...
                    // Wake bg_thread via futex
                    bg_wake_seq.fetch_add(1, std::memory_order_release);
                    futex_wake_priv(reinterpret_cast<int*>(&bg_wake_seq), 1);
                } else {
