...
// io_uring fadvise submission (POC: bg-thread-futex, direction E)
struct IoUringFadvise {
    int ring_fd = -1;
    ...
    IoUringFadvise(unsigned entries = 512) {
        memset(&params, 0, sizeof(params));
        ring_fd = (int)syscall(__NR_io_uring_setup, entries, &params);
        if (ring_fd < 0) throw std::runtime_error("io_uring_setup failed");
        
        unsigned sq_entries = params.sq_entries;
        
        // mmap SQ ring
        size_t sq_ring_sz = params.sq_off.array + sq_entries * sizeof(unsigned);
        void *sq_ring = mmap(nullptr, sq_ring_sz, PROT_READ | PROT_WRITE,
                             MAP_SHARED | MAP_POPULATE, ring_fd, 0);
        if (sq_ring == MAP_FAILED) throw std::runtime_error("mmap SQ ring failed");
        
        // mmap SQEs
        size_t sqes_sz = sq_entries * sizeof(struct io_uring_sqe);
        void *sqes_ptr = mmap(nullptr, sqes_sz, PROT_READ | PROT_WRITE,
                              MAP_SHARED | MAP_POPULATE, ring_fd, 0x10000000ULL);
        if (sqes_ptr == MAP_FAILED) throw std::runtime_error("mmap SQEs failed");
        
        sq_tail = (unsigned*)((char*)sq_ring + params.sq_off.tail);
        sq_head = (unsigned*)((char*)sq_ring + params.sq_off.head);
        sq_mask = *(unsigned*)((char*)sq_ring + params.sq_off.ring_mask);
    ...
    // Submit a single fadvise WILLNEED request
    void submit_fadvise(int fd, uint64_t offset, uint32_t len) {
        unsigned tail = *sq_tail;
        unsigned head = *sq_head;
        if (tail - head >= sq_mask) {
            // Ring full — flush and drain CQ first
            submit();
            drain_cq();
        }
        tail = *sq_tail;
    ...
    void submit() {
        ...
        if (pending > 0) {
            syscall(__NR_io_uring_enter, ring_fd, pending, 0, 0, nullptr);
        }
    }
    
    // Drain completion queue (non-blocking)
    void drain_cq() {
        // Need CQ ring pointers for draining
        // For simplicity, just call io_uring_enter with to_submit=0
        // which processes pending CQEs
        // The CQ ring will be drained on next submit
        syscall(__NR_io_uring_enter, ring_fd, 0, 0, 0, nullptr);
    }
};
#include <mutex>
#include <array>
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            if (kL4Willneed && !pages_needed.empty() && vec_blocks_fd_ >= 0) {
                if (kWillneedBg) {
                    // E-iouring: async fadvise via io_uring (no bg_thread needed)
                    // POC: bg-thread-futex, direction E
                    static IoUringFadvise ring;
                    static const bool kPageMergeBg = std::getenv("PAGE_MERGE_BG") && std::atoi(std::getenv("PAGE_MERGE_BG")) != 0;
                    
                    if (kPageMergeBg && pages_needed.size() > 1) {
                        // Merge contiguous pages
                        std::vector<uint32_t> sorted(pages_needed.begin(), pages_needed.end());
                        ...
                        while (j < sorted.size()) {
                            ...
                            ring.submit_fadvise(vec_blocks_fd_, (uint64_t)start << 12, (uint32_t)((end - j) << 12));
                            j = end;
                        }
                    } else {
                        for (uint32_t pg : pages_needed) {
                            ring.submit_fadvise(vec_blocks_fd_, (uint64_t)pg << 12, 4096);
                        }
                    }
                    ring.submit();  // Flush to kernel (async, returns immediately)
                }  // io_uring path handles both BG=1 (above)
            }
