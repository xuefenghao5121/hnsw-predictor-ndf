...
// POC ssd-parallelism-io: per-thread io_uring ring
...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            if (!vec_ring_) {
                // io_uring init failed for this thread, fallback to pread
                for (uint32_t pg : pages_needed) {
                    char* raw = nullptr;
                    posix_memalign((void**)&raw, 4096, 4096);
                    auto buf = std::unique_ptr<char[]>(raw);
                    ssize_t r = pread(vec_blocks_fd_, buf.get(), 4096, (off_t)pg << 12);
                    if (r == 4096) {
                        for (const auto& c : io_cands) {
                            if (c.page0 == pg && !c.cross) consider(c.nid, reinterpret_cast<const float*>(buf.get() + c.oip));
                        }
                    }
                }
            ...
            // === POC ssd-parallelism-io: CQE peeking (completion-order processing) ===
            // 不等全部完成，CQE 到达后立即计算距离
            ...
            // 预建 page -> io_cands 索引（哪些候选需要这个 page）
            ...
            std::vector<bool> page1_ready(io_cands.size(), false);  // for cross-page
            ...
            while (done < total) {
                ...
                for (const auto& cqe : results) {
                    ...
                    if (cqe.res != expect) {
                        ...
                        // Page ready - immediately process candidates that depend on it
                        markPageReady(p0);
...
bool DiskHNSW::buildFineRerank(const std::string& blocks_path, uint32_t num_nodes) {
    ...
    // POC: vec_ring_ is now thread_local, init lazily per-thread
    // buildFineRerank only sets up tables; each thread creates its own ring on first use
    vec_blocks_fd_ = fd;
