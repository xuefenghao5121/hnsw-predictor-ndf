...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        // --- LLSP Profiling: dump Phase A 候选 PQ 距离 + ID ---
        static const bool kProfileLlsp = std::getenv("PROFILE_LLSP") && std::atoi(std::getenv("PROFILE_LLSP")) != 0;
        if (kProfileLlsp) {
            // 输出格式: [LLSP] qid=<query_idx> n_coarse=<N> d0=<dist0> d1=<dist1> ... d9=<dist9>
            //          gap=<gap_ratio> std=<std> mean=<mean> ids=<id0>,<id1>,...,<idN-1>
            static std::atomic<size_t> query_idx_{0};
            size_t qid = query_idx_++;  // atomic counter
            ...
            double mean = sum / dump_n;
            double stdv = sqrt(sum2 / dump_n - mean * mean);
            float d0 = coarse_sorted[0].first;
            float d9 = coarse_sorted.size() > 9 ? coarse_sorted[9].first : 0;
            float dk_val = coarse_sorted.size() > k ? coarse_sorted[k-1].first : 0;
            float dk1_val = coarse_sorted.size() > k ? coarse_sorted[k].first : 0;
            float gap = (dk_val > 0) ? (dk1_val / dk_val) : 1.0f;
            fprintf(stderr, "[LLSP] qid=%zu n=%zu d0=%.2f d9=%.2f dk=%.2f dk1=%.2f gap=%.4f mean=%.2f std=%.2f",
                    qid, coarse_sorted.size(), d0, d9, dk_val, dk1_val, gap, mean, stdv);
            // dump 前 100 个候选 ID (转换为 label 以匹配 GT)
            fprintf(stderr, " ids=");
            for (size_t i = 0; i < std::min(dump_n, (size_t)500); i++) {
                uint32_t newId = coarse_sorted[i].second;
                uint32_t oldId = new_to_old_[newId];
                uint64_t label = graph_.labels[oldId];
                fprintf(stderr, "%lu%c", (unsigned long)label, i < std::min(dump_n, (size_t)500)-1 ? ',' : '\n');
            }
        }
        ...
        if (kLearnedEf && coarse_sorted.size() > (size_t)k) {
            ...
            double stdv = sqrt(sum2 / feat_n - mean * mean);
            float cv = (float)(stdv / mean);
            float r01 = (float)(coarse_sorted[0].first / mean);
            ...
            float feat[11] = {
                (float)coarse_sorted.size(), // n_coarse
                coarse_sorted[0].first,      // d0
                coarse_sorted.size() > 9 ? coarse_sorted[9].first : 0, // d9
                dk_val, dk1_val, gap,
                (float)mean, (float)stdv, cv, r01, r09
            };
            
        static const float kGbdtMargin = []() { const char* e = std::getenv("GBDT_MARGIN"); return e ? std::stof(e) : 1.0f; }();
