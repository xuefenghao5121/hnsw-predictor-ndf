    ...
    , dim_param_(0)
{
    ...
    // L4_EVICT_META: evict graph/BFS page cache after CSR built
    ...
    if (kL4EvictMeta) {
        ...
        if (gfd >= 0) { posix_fadvise(gfd, 0, 0, POSIX_FADV_DONTNEED); close(gfd); }
        int bfd = open(bfs_path.c_str(), O_RDONLY);
        if (bfd >= 0) { posix_fadvise(bfd, 0, 0, POSIX_FADV_DONTNEED); close(bfd); }
        std::cerr << "[L4] Evicted metadata page cache: graph+bfs" << std::endl;
    }
}
...
DiskHNSW::searchLayer0(uint32_t entry_new_id, const float* query, size_t ef,
    ...
    std::unordered_set<uint32_t>& piped_pages = pipe_piped_pages_; piped_pages.clear();
    std::unordered_map<uint32_t, int>& pipe_page_bufidx = pipe_page_bufidx_; pipe_page_bufidx.clear();
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            for (uint32_t nid : cand_ids) {
                ...
                // Check flat_vec_cache before going to I/O (promote l4-cache-mgmt, DEC-068)
            ...
            if (kPipeFine && pipe_ring_) {
                ...
                if (pipe_hits > 0) {
                    fprintf(stderr, "[PipeFine] pipe_hits=%d (piped=%zu)\n",
                            pipe_hits, pipe_piped_pages_.size());
                }
            }
