#pragma once
#ifndef DISK_HNSW_PIPE_H
#define DISK_HNSW_PIPE_H
// disk_hnsw_pipe.h - Patched DiskHNSW for I/O Pipelining POC (BEH-021 draft)
// DO NOT use in production - POC only
// 这个文件替代 include/disk_hnsw.h, 通过 -include 在编译时注入
//
// 设计要点：
...
private:
    ...
    // ---- I/O Pipelining (PIPE_FINE=1, BEH-021 draft) ----
    // thread_local: 每线程独立 io_uring 实例, 多线程安全
    static thread_local std::unique_ptr<IoUring> pipe_ring_;
    static constexpr int kPipeRingEntries = 256; // pipe_ring_ SQE 深度
