...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            // === POC: fine-rerank-incremental ===
            // FINE_INCREMENTAL=1: 分批增量 pread + 批间早终止
            static const bool kFineIncremental = std::getenv("FINE_INCREMENTAL") && std::atoi(std::getenv("FINE_INCREMENTAL")) != 0;
            static const int kFineBatchSize = []() { const char* e = std::getenv("FINE_BATCH_SIZE"); return e ? std::atoi(e) : 16; }();
            static const bool kFineEarlyStop = std::getenv("FINE_EARLY_STOP") && std::atoi(std::getenv("FINE_EARLY_STOP")) != 0;
            static const float kFineStopMargin = []() { const char* e = std::getenv("FINE_STOP_MARGIN"); return e ? std::stof(e) : 1.5f; }();
            static const int kFineStopStreak = []() { const char* e = std::getenv("FINE_STOP_STREAK"); return e ? std::atoi(e) : 0; }();
            // Stats for early stop
            static long inc_skip_count = 0, inc_total_count = 0;
            static long inc_batches_total = 0, inc_queries_total = 0;

            if (kFinePread && kFineIncremental) {
                // ---- 增量 pread 路径: 分批读取 + 批间早终止 ----
                auto tp0 = std::chrono::high_resolution_clock::now();
                char tmp_vec_pread[512];

                // io_cands is already sorted by PQ distance (ascending via cand_ids)
                size_t total_cands = io_cands.size();
                size_t processed = 0;
                int streak_no_improve = 0;
                bool early_stopped = false;

                while (processed < total_cands) {
                    size_t batch_end = std::min(processed + kFineBatchSize, total_cands);

                    // Collect pages for this batch
                    std::set<uint32_t> batch_pages;
                    for (size_t i = processed; i < batch_end; i++) {
                        batch_pages.insert(io_cands[i].page0);
                        if (io_cands[i].cross) batch_pages.insert(io_cands[i].page0 + 1);
                    }

                    // pread all pages in this batch
                    std::unordered_map<uint32_t, std::unique_ptr<char[]>> batch_page_cache;
                    batch_page_cache.reserve(batch_pages.size());
                    for (uint32_t pg : batch_pages) {
                        ...
                        if (r == 4096) batch_page_cache[pg] = std::move(buf);
                    }

                    // Compute L2 for candidates in this batch
                    int batch_hits = 0;  // how many entered top-K
                    float prev_topk_dist = refined.size() >= k ? refined.top().first : std::numeric_limits<float>::max();

                    for (size_t i = processed; i < batch_end; i++) {
                        const auto& c = io_cands[i];
                        auto it0 = batch_page_cache.find(c.page0);
                        if (it0 == batch_page_cache.end()) continue;
                        const char* p0 = it0->second.get();
                        const float* vec;
                        if (!c.cross) {
                            vec = reinterpret_cast<const float*>(p0 + c.oip);
                        } else {
                            auto it1 = batch_page_cache.find(c.page0 + 1);
                            if (it1 == batch_page_cache.end()) continue;
                        ...
                        // Track if this candidate improved top-K
                        float cur_topk_dist = refined.size() >= k ? refined.top().first : std::numeric_limits<float>::max();
                        if (cur_topk_dist < prev_topk_dist) {
                            batch_hits++;
                            streak_no_improve = 0;
                        } else {
                            streak_no_improve++;
                    ...
                    // Early stop check
                    if (kFineEarlyStop && refined.size() >= k) {
                        float kth_dist = refined.top().first;  // current k-th exact distance
                        // Check: next candidate's PQ distance vs k-th exact distance
                        if (processed < total_cands) {
                            // Find the next io_cand's PQ distance from coarse_sorted
                            // io_cands is built from cand_ids which comes from coarse_sorted (ascending PQ dist)
                            // Map io_cands index back to coarse_sorted index
                            // cand_ids[cache_miss_start + processed] corresponds to coarse_sorted[...]
                            // Actually, io_cands preserves the order of cand_ids (PQ ascending)
                            // We need the PQ distance of the next candidate
                            // Simplest: use the PQ distance of the last processed candidate as proxy
                            // More precise: track via coarse_sorted. For now use streak-based + margin.

                            bool should_stop = false;

                            // Condition 1: streak-based (consecutive non-improving)
                            if (kFineStopStreak > 0 && streak_no_improve >= kFineStopStreak) {
                                should_stop = true;
                            }

                            // Condition 2: margin-based
                            // The PQ distance of candidates only increases (sorted ascending)
                            // If current k-th exact dist × margin < next candidate's PQ distance, stop
                            // But we don't have PQ dist here directly. Use batch_hits as proxy:
                            // If batch_hits == 0 for this batch AND we've processed >= k candidates, stop
                            if (batch_hits == 0 && processed >= (size_t)k * 2) {
                                should_stop = true;
                            }

                            if (should_stop) {
                                inc_skip_count += (total_cands - processed);
                                early_stopped = true;
                                break;
                            }
                        }
                ...
                if (kProfile) {
                    ...
                    double total_us = std::chrono::duration<double, std::micro>(tp2 - tp0).count();
                    acc_a += std::chrono::duration<double, std::micro>(tpA - tp0).count();
                    acc_wait += total_us * 0.8;  // approximate
                    acc_rerank += total_us * 0.2;
                    acc_n++;
                    if (acc_n % 200 == 0) {
                        double skip_rate = inc_total_count > 0 ? (double)inc_skip_count / inc_total_count * 100 : 0;
                        double avg_batches = inc_queries_total > 0 ? (double)inc_batches_total / inc_queries_total : 0;
                        fprintf(stderr, "[PROFILE_INC] n=%ld total=%.0fus | skip_rate=%.1f%% avg_batches=%.1f\n",
                                acc_n, total_us, skip_rate, avg_batches);
                    }
                }
            } else if (kFinePread) {
                // ---- 原始 pread 路径 (线程安全, 多线程并发用) ----
                auto tp0 = std::chrono::high_resolution_clock::now();
