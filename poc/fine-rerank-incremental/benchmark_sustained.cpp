...
//
// Motivation (DEC-083): A single fixed query set gets fully absorbed by the page
// cache during warmup, so measured QPS reflects in-memory search rather than the
// disk-resident design target. This harness samples a fresh random subset from the
// official SIFT query pool every round, so the cumulative working set keeps growing
// and the measurement converges to sustained I/O-bound behaviour.
//
// Critically: there is NO query warmup pass. A warmup over the measured queries is
// precisely what manufactures the cache-hit illusion. Only a CPU-frequency spin is
// performed. Optional --warmup rounds use DISJOINT seeds and are excluded from stats.
//
// Usage:
...
int main(int argc, char** argv) {
    ...
    if (stats_per_round.size() >= 2) {
        ...
        std::cout << "Decay:         " << std::setprecision(1)
                  << (100.0 * (first - last) / first) << "%" << std::endl;
    }

    // Machine-readable tail for scripted sweeps.
    ...
    std::cout << "CSV_AGG," << agg_queries << "," << std::setprecision(6) << agg_seconds
