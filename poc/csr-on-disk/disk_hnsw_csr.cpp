...
void DiskHNSW::buildInMemoryAdjacency() {
    ...
    // === POC: csr-on-disk - mmap CSR compact ===
    static const char* csr_file = std::getenv("CSR_FILE_PATH");
    if (csr_file && csr_file[0]) {
        // Write CSR compact to file
        std::string csr_path(csr_file);
        std::ofstream csr_out(csr_path, std::ios::binary);
        csr_out.write(reinterpret_cast<const char*>(adj_csr_compact_.data()),
                      adj_csr_compact_.size());
        csr_out.close();
        std::cout << "  [CSR-on-disk] Written CSR compact to " << csr_path
                  << " (" << adj_csr_compact_.size() << " bytes)" << std::endl;

        // mmap the file
        int fd = open(csr_path.c_str(), O_RDONLY);
        if (fd < 0) {
            std::cerr << "ERROR: Cannot open CSR file: " << csr_path << std::endl;
            return;
        }
        size_t file_size = adj_csr_compact_.size();
        void* mapped = mmap(nullptr, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
        if (mapped == MAP_FAILED) {
            std::cerr << "ERROR: mmap failed for CSR file" << std::endl;
            close(fd);
            return;
        }
        ...
        // Store mmap state
        csr_mmap_ptr_ = static_cast<const uint8_t*>(mapped);
        csr_mmap_fd_ = fd;
        csr_mmap_size_ = file_size;
        csr_on_disk_ = true;

        std::cout << "  [CSR-on-disk] mmap'd " << file_size << " bytes, "
                  << "freed " << file_size / (1024.0*1024) << "MB from RSS" << std::endl;
    }
...
uint32_t DiskHNSW::decodeCsrNeighbors(uint32_t new_id) {
    ...
    const uint8_t* p = csr_on_disk_ ? (csr_mmap_ptr_ + byte_start) : (adj_csr_compact_.data() + byte_start);
