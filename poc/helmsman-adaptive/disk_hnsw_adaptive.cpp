...
std::vector<DiskHNSW::SearchResult> DiskHNSW::searchKnn(const float* query, size_t k) {
    ...
    if (kTwoStage && pq_enabled_) {
        ...
        // === HELMSMAN adaptive: PQ gap profiling + adaptive EF ===
        // PROFILE_GAP=1: 输出 per-query gap 分布到 stderr
        // ADAPTIVE_EF=1: 根据 gap 调整 Phase B 候选数
        static const bool kProfileGap = std::getenv("PROFILE_GAP") && std::atoi(std::getenv("PROFILE_GAP")) != 0;
        static const bool kAdaptiveEf = std::getenv("ADAPTIVE_EF") && std::atoi(std::getenv("ADAPTIVE_EF")) != 0;
        static const bool kEarlyTerminate = std::getenv("EARLY_TERMINATE") && std::atoi(std::getenv("EARLY_TERMINATE")) != 0;

        // 收集 coarse 结果 + PQ 距离 (coarse 是最小堆, pop 顺序: 距离从小到大)
        std::vector<std::pair<float, uint32_t>> coarse_sorted; // (pq_dist, node_id), 升序
        coarse_sorted.reserve(coarse.size());
        while (!coarse.empty()) {
            coarse_sorted.push_back(coarse.top());
            coarse.pop();
        }

        // R0 profiling: gap = dist[k] / dist[k-1] (第 K 个与第 K-1 个的距离比)
        float gap_ratio = 0.0f;
        float dk = 0.0f, dk1 = 0.0f;
        if (coarse_sorted.size() > k) {
            dk = coarse_sorted[k-1].first;
            dk1 = coarse_sorted[k].first;
            gap_ratio = (dk > 0) ? (dk1 / dk) : 1.0f;
        }
        if (kProfileGap) {
            // 输出: query_idx, coarse_size, gap_ratio, dist[0], dist[k-1], dist[k], dist[-1]
            fprintf(stderr, "[GAP] n_coarse=%zu gap_ratio=%.4f d0=%.2f d%d=%.2f d%d=%.2f d_last=%.2f\n",
                    coarse_sorted.size(), gap_ratio,
                    coarse_sorted.empty() ? 0 : coarse_sorted[0].first,
                    (int)k-1, dk,
                    (int)k, dk1,
                    coarse_sorted.back().first);
        }

        // R1 adaptive: 根据 gap_ratio 调整 Phase B 候选数
        // gap_ratio 接近 1.0 = 困难 query (距离接近, 需要更多候选)
        // gap_ratio 大 = 容易 query (距离差距大, 少候选即可)
        static const float kEasyGap = []() { const char* e = std::getenv("ADAPTIVE_EASY_GAP"); return e ? std::stof(e) : 1.15f; }();
        static const float kHardGap = []() { const char* e = std::getenv("ADAPTIVE_HARD_GAP"); return e ? std::stof(e) : 1.03f; }();
        ...
        size_t adaptive_limit = coarse_sorted.size();  // 默认: 全用
        if (kAdaptiveEf) {
            ...
            if (kProfileGap) {
                fprintf(stderr, "[ADAPTIVE] gap=%.4f -> limit=%zu (easy>=%.2f, hard<=%.2f)\n",
                        gap_ratio, adaptive_limit, kEasyGap, kHardGap);
            }
        }

        std::vector<uint32_t> cand_ids;
        cand_ids.reserve(adaptive_limit);
        for (size_t i = 0; i < adaptive_limit; i++) {
            cand_ids.push_back(coarse_sorted[i].second);
        }
        ...
        if (kFineRerank && fine_rerank_ok_) {
            ...
            // --- HELMSMAN 层次 B: Fine Rerank 早终止 ---
            static const int kEarlyTermLimit = []() {
                const char* e = std::getenv("EARLY_TERMINATE");
                return e ? std::atoi(e) : 0;  // 0=关闭, >0=连续无改善阈值
            }();
            int early_term_count = 0;  // 连续无改善计数
            bool early_terminated = false;
            long et_saved_total = 0;  // 统计: 早终止节省的候选数

            auto consider = [&](uint32_t nid, const float* vec) {
                float d = l2Distance(query, vec);
                bool improved = false;
                if (refined.size() < k) { refined.emplace(d, nid); improved = true; }
                else if (d < refined.top().first) { refined.pop(); refined.emplace(d, nid); improved = true; }
                if (improved) early_term_count = 0;
                else early_term_count++;
            };
            ...
            if (kFinePread) {
                ...
                for (size_t ci = 0; ci < io_cands.size(); ci++) {
                    // 层次 B: 早终止检查
                    if (kEarlyTermLimit > 0 && early_term_count >= kEarlyTermLimit && refined.size() >= k) {
                        early_terminated = true;
                        et_saved_total += (long)(io_cands.size() - ci);
                        break;
                    }
