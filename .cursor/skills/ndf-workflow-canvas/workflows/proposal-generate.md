...
4. Run the unique CLI. Empty intent MUST NOT dispatch. Report summary; wait for 「派发」.
5. `dispatch-send` → OpenClaw drafts one `spec/open/proposal-*.md` (`Status: Pending confirmation`).
6. Stop at **已确认**. MUST NOT create `poc/` before that phrase. MUST NOT write `spec/meta/open/` from Command Agent.
7. Closeout: action-commit + action-finish + snapshot.
