...
Command Agent only builds `control-pack`; `afterShellExecution` hook sends OpenClaw.

## Sequence

1. GIT INPUT checkout of `remote_branch`.
2. Prefer `--resume` when the gate Episode is active.
3. `control-pack --task gate_pipeline --json` then **STOP**.
4. Hook: `dispatch-send` → OpenClaw → wait response → completion → action-commit → snapshot.
5. OpenClaw (worker): each gate drafts `GATES.md` pending rows; MUST NOT set `approved_by`. Wait for the exact human phrase.
