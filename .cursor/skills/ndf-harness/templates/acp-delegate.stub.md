...
只允许修改: poc/⟨topic⟩/
禁止: src/、stable must SLA、spec/models/ 生产补丁、spec/meta/
任务: ⟨TBD⟩
证据写到: poc/⟨topic⟩/NOTES.md 与 ndf/evidence/
完成后输出摘要（文件列表 + 如何复现）。
```
...
禁止改 00-charter / 10-architecture / L0-L1 / spec/meta/。
完成后输出摘要。
```
...
```text
【track=process】无 src 任务；不得改 spec/meta/（由 OpenClaw 维护）。
若收到此委派，回复无需编码并退出。
```
