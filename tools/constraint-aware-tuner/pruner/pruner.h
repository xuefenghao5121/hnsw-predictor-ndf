// pruner.h — constraint-aware-tuning: cheap feasibility pruning ([[BEH-028]])
//
...
//
// Offline DSE only ([[ARCH-009]]); does not enter the search hot path.

#ifndef CAT_TUNER_PRUNER_H
#define CAT_TUNER_PRUNER_H
...
// configs. Returns pruning statistics for reporting (TOPIC/NOTES success criteria #3).
inline PruneStats prune_infeasible(const std::vector<TuningKnobs>& in,
    ...
    // drop Pareto-dominated (O(n^2) is fine for the small offline candidate set)
