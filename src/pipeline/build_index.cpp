// build_index.cpp - 构建分层 Vamana 索引 (Trunk 默认建图入口)
//
// 自 promote hierarchical-vamana 起，替换原 hnswlib 整图建图：用
//   "HNSW 几何层分配 + 每层 Vamana（GreedySearch + RobustPrune, α）"
//   建图，导出 GraphStructure（与原 extract_graph 输出同格式），
//   使现有 bfs_reorder / write_blocks / write_blocks_veconly / gen_route
//   流水线与 DiskHNSW 搜索侧保持不变。
//
...
//
// 用法:
//   ./build_index <data.fvecs> <out_graph.bin> [--max-points N] [--self-test K]
//     --max-points N : 仅对前 N 点建图（smoke / 子集运行）
//     --self-test K  : 建图后对前 K 点做贪心下降 + L0 beam 搜索
//                      （leave-one-out）并报告 Recall@10 vs 暴力对照。
//   Env knobs: HV_M, HV_R0, HV_RUP, HV_BEAM, HV_ALPHA, HV_ALPHA2,
//              HV_ROUNDS, HV_SEED。
