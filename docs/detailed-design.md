...
> **口径说明**: SIFT1M sustained (官方 10K query 池, 禁预热) 为权威口径 ([[CON-SLA-020]])。
> 下方表格为 cache-warmed 口径 (200q + 预热), 高估 1.73-7.60x, 仅作回归护栏。
> Sustained: 512MB 16T = 6,694 QPS / 96.00% recall; 256MB 16T = 2,456 QPS / 96.00% recall。
> ADAPTIVE_EF (BEH-033) sustained +12.5~31.4%; LEARNED_EF (BEH-034) sustained 收益不成立 (需重训练)。

| 配置 | 指标 | 目标 | 实测 (cache-warmed) | Sustained |
|------|------|------|------|------|
| 512MB 1T | Recall@10 | ≥95% | 95.75% | 96.00% |
| 512MB 1T | QPS | ≥2000 | 3,366 | 1,493 (agg) |
| 512MB 16T | QPS | ≥20000 | 30,332 | 4,475 (agg) |
| 256MB 16T | QPS | ≥10000 | 18,675 | 2,092 (agg) |
| 256MB | RSS | ≤256MB | 223MB | ~223MB |
| 所有配置 | oom | =0 | 0 | 0 |

**对比 hnswlib**: hnswlib 需 732-763MB RSS (OOM@512MB)，DiskHNSW 绝对内存 33% of hnswlib。
...
### 4.3 扩展性 (cache-warmed 口径, 回归护栏)

> ⚠️ 以下为 cache-warmed 数字, 高估 1.73-7.60x。Sustained 稳态: 512MB 16T = 6,694, 256MB 16T = 2,456。

| 线程数 | 512MB QPS (cw) | 256MB QPS (cw) | 512MB Sustained | 256MB Sustained | Scaling (cw) |
|--------|-----------|-----------|------|------|---------|
| 1T | 3,366 | 2,830 | 1,493 | 1,090 | 1.0× |
| 4T | 9,041 | 7,721 | 3,920 | 2,201 | 2.7× |
| 8T | 14,901 | - | 4,525 | 2,185 | 4.4× |
| 12T | 18,459 | 13,799 | - | - | 5.5× |
| 16T | 30,332 | 18,675 | 4,475 | 2,092 | 9.0× |
| 24T | 29,738 | - | - | - | 8.8× |
